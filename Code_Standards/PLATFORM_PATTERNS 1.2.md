...
**Revision 1.2 — 2026-07-17.** Corrects §12's capability list to the shipped recovery menu and adds
the EXEC-wrap migration rule to §11 — both from Observatory field feedback. History at the end of the file.
...
- **Recovery scope only**; every action **audited** to the shared log.
...
- **Paired with the startup guard** (§11): the service *refuses to start* when the database is behind;
  this tool is the named remedy. Guard + explicit tool = no accidental migrations, no mystery
  failures.
...
*Finish-guide tie:* §2 (guard-rails: refuse + name the remedy; destructive ops confirm with
consequences), §6 (failure states carry the fix, not just the fact).
