...
**Revision 1.1 — 2026-08-21.** Adds the §3 disclosure-binding gotcha (null-equality false-"open") and
its appendix cross-reference. History at the end of the file. The filename carries the current
revision; a copy without this line is pre-1.1.
...
**review any screen against**, largely stack-agnostic and stable. Its companion, **`PLATFORM_PATTERNS 1.2.md`**
(the Platform Patterns doc — filename carries its current revision),
...
- **Expandable sections use disclosure controls**: a chevron that points right when collapsed and
  ...
  *own* the section it reveals. A plain action button that secretly expands a row gives the user
  ...
  - **Gotcha — bind `.open` to a real boolean, never to an id-equality.** The natural binding for a
    selection-scoped expander is `[class.open]="panelFor === selectedId"` ("the open panel is the
    selected row's"). With nothing selected and nothing open, *both ids are null*, and `null === null`
    is `true` — so on a fresh load the chevron renders rotated and `aria-expanded="true"` while
    nothing is open; selecting a row "fixes" it, deselecting breaks it again. Guard so null never
    matches — `panelFor !== null && panelFor === selectedId` — ideally as a getter
    (`get drawerOpen(): boolean`) that both `[class.open]` and `[attr.aria-expanded]` bind to. The
    boolean matters for a11y too: a `null` fed to `aria-expanded` *drops the attribute* instead of
    saying `"false"`. (Field-found across four FDP pages; fixed in commit `78ab26a`.)
...
| `button.disclosure` + `.chev` | Expander control: chevron points right, rotates 90° and turns accent when `.open` — the trigger visibly owns the section it reveals. Bind `.open`/`aria-expanded` to a real boolean (see the §3 null-equality gotcha). | §3 |
...
/* Inline detail drawer rendered as a full-width table row under the selected entry. The cell has
   no padding of its own; the drawer sits inside it with its own margin. Opens and closes instantly
   (a height animation inside a <table> cell is unreliable — the row would strand collapsed). */
.drawer-cell { padding: 0; }
.drawer-cell .drawer { margin: 8px 12px 12px; }
...
| Rev | Date | Changes |
|---|---|---|
| 1.1 | 2026-08-21 | §3: disclosure-control gotcha — bind `.open`/`aria-expanded` to a null-guarded boolean, never to an id-equality (`panelFor === selectedId` is `true` when both are null → false-"open" on fresh load; field-found on four FDP lists, fixed in FDP `78ab26a`); appendix `button.disclosure` row cross-references it. First stamped revision; also checked into Lee's code-standards project as the canonical copy. |
| 1.0 | 2026-07-10 | The handoff version: §1–8 + finish checklist, the two-lens preamble with →Patterns cross-links, the Design System appendix (tokens, component classes, reusable components/services catalog), the embedded copy-ready stylesheet, and the self-contained-handoff note. |
