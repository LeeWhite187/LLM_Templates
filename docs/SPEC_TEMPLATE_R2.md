...
**Status:** Active
**Created:** 2026-05-06
**Related Documents:** METHODOLOGY.md, SPEC_RETROFIT_INSTRUCTION.md
...
Open items fall into three dispositions, indicated by the flag in the title:
...
Open items are resolved by either updating the spec to address them (closing the item with a brief "resolved: see §[X]" note) or by deciding the item is permanently deferred (closing with a brief rationale). Closed items remain in the document with their resolution noted; the number is never reused.]
