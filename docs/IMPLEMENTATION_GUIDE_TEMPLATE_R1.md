...
**Reference durable spec elements.** When the guide cites the spec, prefer Key Decisions (KD-NN) and section numbers (§X.Y) — both are stable across spec revisions. Avoid citing Open Items (OI-NN) directly. OIs are workspace items that may be tombstoned in future spec revisions; references to them will rot. If a current OI is meaningful to the guide, wait until it is resolved into a KD or section change before citing it.
